...
class ZbjAdapter(ZbjMockAdapter):
    ...
    def pull_orders(self, payload: dict[str, Any]) -> dict[str, Any]:
        source_mode = (settings.zbj_orders_source_mode or "mock").strip().lower()
        if source_mode != "http":
        ...
        try:
            ...
            return self.make_response(
                ...
                message=f"zbj pull_orders fetched {len(records)} records from http source",
            ...
            message = f"zbj pull_orders http source failed: {exc}"
            return self.make_response(
                success=False,
                accepted=False,
                final=True,
                code="ZBJ_PULL_ORDERS_ERROR",
                message=message[:500],
    ...
    def _fetch_order_records(self, payload: dict[str, Any]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
        endpoint = (settings.zbj_orders_endpoint or "").strip()
        if endpoint == "":
            raise ValueError("ZBJ_ORDERS_ENDPOINT is empty")
        ...
        access_token = (settings.zbj_access_token or "").strip()
        if access_token != "":
            headers["Authorization"] = f"Bearer {access_token}"

        app_key = (settings.zbj_app_key or "").strip()
        if app_key != "":
            headers["X-App-Key"] = app_key

        timeout = max(1.0, float(settings.zbj_http_timeout_seconds))
    ...
    def _build_query_params(self, payload: dict[str, Any]) -> dict[str, str]:
        ...
        raw_extra = (settings.zbj_orders_extra_params_json or "").strip()
        if raw_extra != "":
            try:
                parsed_extra = json.loads(raw_extra)
            except json.JSONDecodeError as exc:
                raise ValueError("ZBJ_ORDERS_EXTRA_PARAMS_JSON is not valid json object") from exc

            if not isinstance(parsed_extra, dict):
                raise ValueError("ZBJ_ORDERS_EXTRA_PARAMS_JSON must be json object")
